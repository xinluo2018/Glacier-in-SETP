...
NUMB_DEMs=$(ls -d $PATH_DEMs | wc | awk '{print $1}')
echo "Numb of DEMs to process: " $NUMB_DEMs
...
WEST=91; EAST=99; SOUTH=27; NORTH=32
EXTENT="$WEST $NORTH $EAST $SOUTH"
PATH_SUBS=$DIR_DATA/dems_mosaic_subs.tif  ## 
...
gdalwarp -s_srs "$TSRS_WGS84" -t_srs "$TSRS_UTM" -r bilinear -co COMPRESS=LZW -co TILED=YES $PATH_SUBS $DIR_DATA/dems_mosaic_subs_utm.tif
